...
public interface UserMapper {
	/*
	 * <insert id="createUser" parameterType="com.itwill.user.User">
	 	INSERT INTO userinfo VALUES (#{user_id}, #{user_pw}, #{user_name}, #{user_email}, #{user_phone}, #{user_freshness}, #{user_point}, #{user_profile}) </insert>
	 */
	 ...
	 * 
	 * @Delete("delete userinfo where userid=#{userId}") public int remove(String
	 * userId);
	 * 
	 * @Select("select userid,password,name,email from userinfo where userid=#{userId}"
	 * ) public User findUser(String userId);
	 * 
	 * @Select("select userid,password,name,email from userinfo") public List<User>
	 * findUserList();
	 * 
	 * @Select("select count(*) cnt from userinfo where userid=#{userId}") public
	 * int existedUser(String userId);
	 */
}