...
public class StudentDao {
	...
	public int insertStudentBySequence2(Student student) {
		...
		int rowCount=
				sqlSession.insert(NAMESPACE+"insertStudentBySequence1",student);
		/* 위에서 insert가 실행되면서, selectkey를 통해 student객체에 seq를 넣어주고,
		 그 student객체로 insert sql을 실행한다.
			따라서, rowCount가 return되고,
			student객체를 통해 studId를 얻을 수 있다.
		 */ 
		sqlSession.close();
